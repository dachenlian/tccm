...
FNType noMorFName[FNSize];		/* used in cutt_dir to see if 0nomor.cdc file is present */
...
static char isRightR8Command(void) {
	if (R8) {
		if (CLAN_PROG_NUM == FREQ || CLAN_PROG_NUM == C_NNLA || CLAN_PROG_NUM == C_QPA || CLAN_PROG_NUM == MORTABLE ||
			CLAN_PROG_NUM == EVAL || CLAN_PROG_NUM == SUGAR) {
			return(TRUE);
		}
	}
	return(FALSE);
...
#define morWsMax 50 // 1024
#define angWsMax 15 // 1024
...
static void createFullFakeMorForSpeaker(char *spTier, char *mor_tier) {
	...
	while (posSp >= 0) {
		...
		for (; posSp >= 0 && uS.isskip(spTier,posSp,&dFnt,MBF) && !uS.isRightChar(spTier,posSp,']',&dFnt,MBF); posSp--) ;
...
void mmaininit(void) {
	...
	if (CLAN_PROG_NUM == MLT || CLAN_PROG_NUM == MLU || CLAN_PROG_NUM == MODREP || CLAN_PROG_NUM == REPEAT ||
		...
		CLAN_PROG_NUM == MORTABLE || CLAN_PROG_NUM == IPSYN ||
	...
	if (CLAN_PROG_NUM == FREQ || CLAN_PROG_NUM == FREQMERGE || CLAN_PROG_NUM == DSS || CLAN_PROG_NUM == PHONFREQ ||
		...
		CLAN_PROG_NUM == CMDI_P || CLAN_PROG_NUM == LAB2CHAT || CLAN_PROG_NUM == MORTABLE ||
		CLAN_PROG_NUM == FLO || CLAN_PROG_NUM == IPSYN || CLAN_PROG_NUM == SCRIPT_P ||
...
//			puts("+d41: outputs in SPREADSHEET format with each matched keyword listed one per column");
	...
	if (option_flags[CLAN_PROG_NUM] & SP_OPTION) {
		...
		} else if (CLAN_PROG_NUM == C_NNLA || CLAN_PROG_NUM == C_QPA || CLAN_PROG_NUM == EVAL || CLAN_PROG_NUM == KIDEVAL || CLAN_PROG_NUM == SUGAR) {
	...
	if (!combinput && ByTurn == '\0') {
		if (option_flags[CLAN_PROG_NUM] & UP_OPTION) {
			if (CLAN_PROG_NUM == QUOTES || CLAN_PROG_NUM == VOCD || CLAN_PROG_NUM == EVAL || CLAN_PROG_NUM == MORTABLE ||
				...
				CLAN_PROG_NUM == CHAT2CONLL || CLAN_PROG_NUM == FLUCALC || CLAN_PROG_NUM == C_NNLA)
...
static int work(char *argv[], int argc, FNType *fname) {
	...
	if (!combinput)
		WUCounter = 0L;
			 ...
			 CLAN_PROG_NUM == FLUCALC || CLAN_PROG_NUM == C_NNLA || CLAN_PROG_NUM == C_QPA || CLAN_PROG_NUM == SUGAR ||
...
#else // CLAN_SRV
				if (!stin_override) {
					if (CLAN_PROG_NUM != C_NNLA && CLAN_PROG_NUM != C_QPA && CLAN_PROG_NUM != EVAL && CLAN_PROG_NUM != KIDEVAL &&
						CLAN_PROG_NUM != FLUCALC && CLAN_PROG_NUM != SUGAR) {
