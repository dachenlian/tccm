...
static void MakeSpHeader(void) {
	...
	if (onlydata == 1) {
		...
		if (timedur_ftime1 || !combinput) {
			...
			excelHeader(fpout, newfname, 75);
			excelRow(fpout, ExcelRowStart);
			excelCommasStrCell(fpout, "File,Language,Corpus,Code,Age,Sex,Group,Race,SES,Role,Education,Custom_field,# Words,# Utterances,Duration Time,# Words/Min,# Utts/Min");
			excelRow(fpout, ExcelRowEnd);
		}
	} else {
		fprintf(fpout, " #  Cur|");
