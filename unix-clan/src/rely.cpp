...
static long masterItemCnt, studenItemCnt, studentItemsMatched, studentItemsNotInMaster;
...
void usage() {
	...
	puts("+dmN: Compute student correctness. (+dm1 - first file is master, +dm2 second file is master)");
...
static void rely_pr_result(FILE *fpo) {
	...
	if (isComputeStudentCorrectness) {
		if (isComputeStudentCorrectness == 1) {
			tf = fTotalUtts;
			ts = sTotalUtts;
			fprintf(fpo, "\nmaster \"%s\": # utterances examined: %d\n", FileName1, fTotalUtts);
		...
		if (isComputeStudentCorrectness == 1) {
			fprintf(fpo, "\nmaster \"%s\": # items examined: %ld\n", FileName1, masterItemCnt);
			fprintf(fpo, "student \"%s\": # items examined: %ld\n", FileName2, studenItemCnt);
		} else {
			fprintf(fpo, "\nmaster \"%s\": # items examined: %ld\n", FileName2, masterItemCnt);
			fprintf(fpo, "student \"%s\": # items examined: %ld\n", FileName1, studenItemCnt);
		}
		fprintf(fpo, "# master items matched by student: %ld\n", studentItemsMatched);
		fprintf(fpo, "# master items missed by student: %ld\n", masterItemCnt - studentItemsMatched);
		tf = masterItemCnt;
		...
		fprintf(fpo, "\n# student items found in master: %ld\n", studentItemsMatched);
		fprintf(fpo, "# student items not found in master: %ld\n", studentItemsNotInMaster);
		tf = studenItemCnt;
		ts = studentItemsMatched;
		if (tf != 0.0) {
			kappa = (ts / tf) * 100;
