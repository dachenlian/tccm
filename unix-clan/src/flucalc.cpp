...
static char flucalc_ftime, isSyllWordsList, isWordMode, isPOSMatch, isMorTierFirst;
...
//				excelNumCell(fpout, "%.3f", (ts->pauses_dur/devNum)*100.0000);
				...
				if (ts->closedClassA == 0.0)
					excelStrCell(fpout, "NA");
				else
					excelNumCell(fpout, "%.3f", ts->closedClassD/ts->closedClassA);
...
/*
 ...
 TD = phrase_repets+word_revis+phrase_revis+pauses_cnti+filled_pauses;
 "#TD" = TD
...
void call()	{		/* tabulate array of word lengths */
	int  i, j, wi, wsLen, syllCnt, chrCnt, wwrSqI, wwrSqCnt;
	unsigned char wwrSq[WWRSQMAX];
	char word[BUFSIZ+1], *ws, *wm, *s;
	char isRepSeg, isWWRFound, ismWWRFound, isRepeatFound, isInternalPause;
	long stime, etime;
	double tNum;
	struct flucalc_speakers *ts;
